...
export default function ActiveGroups() {
  const groups = [
    { name: "Goa Trip", balance: "+₹450", status: "Fair", statusType: "fair" },
    { name: "Flatmates", balance: "-₹1,200", status: "Unfair", statusType: "unfair" },
    { name: "Office Lunch", balance: "+₹80", status: "Fair", statusType: "fair" },
    { name: "Weekend Trek", balance: "₹0", status: "Settled", statusType: "settled" },
  ];
        ...
        {groups.map((g) => (
          ...
          >
            <div className="flex items-start justify-between mb-3">
              <span className="font-serif text-lg text-black">{g.name}</span>
              <span className={`text-xs px-2.5 py-1 rounded-full border font-medium ${statusStyles[g.statusType]}`}>
                {g.status}
              </span>
            </div>
            <div className="flex items-baseline gap-1">
              <span className={`text-2xl font-serif ${g.balance.startsWith("-") ? "text-red-500" : g.balance === "₹0" ? "text-gray-400" : "text-emerald-600"}`}>
                {g.balance}
              </span>
